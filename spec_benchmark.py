...
if target_pp_config!=None:
    logits = target_engine.forward(input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask[..., :PREFIX_LEN,:], storage_ids=prefix_storage_ids)
if draft_pp_config!=None:
    draft_engine.forward(input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask[..., :PREFIX_LEN,:], storage_ids=prefix_storage_ids)
...
if global_rank==0:
    ...
    while True:
        ...
        if control_tensor.item() == -1:
            break
        elif control_tensor.item() == 1 and target_pp_config!=None:
            input_ids=torch.full((1,1),0, device=DEVICE)
            position_ids = torch.full((1,1),seq_offset, device=DEVICE)
            storage_ids = torch.tensor(seq_offset, device=DEVICE)
            target_engine.forward(input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask[..., seq_offset,:], storage_ids=storage_ids)
            seq_offset+=1
        elif control_tensor.item() == 2 and draft_pp_config!=None:
            input_ids=torch.full((1,1),0, device=DEVICE)
            position_ids = torch.full((1,1),seq_offset, device=DEVICE)
            storage_ids = torch.tensor(seq_offset, device=DEVICE)
            draft_engine.forward(input_ids=input_ids, position_ids=position_ids, attention_mask=attention_mask[..., seq_offset,:], storage_ids=storage_ids)
            seq_offset+=1