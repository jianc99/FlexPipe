...
dataloader = DataLoader(tokenized_dataset_eval, batch_size=args.B, collate_fn=data_collator, shuffle=False, drop_last=True)
...
draft_model =  LLM_Pipeline(max_length=MAX_LEN, model_name=DRAFT_MODEL_NAME, device=DEVICE, batch_size=args.B, dtype=torch.float16, pp_config=draft_pp_config, type="spec", last_stage_rank_0=draft_last_stage_rank0, cg_list=cg_list_draft)
target_model = LLM_Pipeline(max_length=MAX_LEN, model_name=TARGET_MODEL_NAME, device=DEVICE, batch_size=args.B, dtype=torch.float16, pp_config=target_pp_config, type="spec", last_stage_rank_0=target_last_stage_rank0, cg_list=cg_list_target)
if args.Mode == "fast":
